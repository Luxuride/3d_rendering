        ...
        egui::SidePanel::right("right_panel")
            ...
            .show(ctx, |ui| {
                ...
                if let Some(model_index) = selected_model {
                    ...
                    let (device, queue) = {
                        let renderer = self.get_renderer().read().unwrap();
                        (renderer.get_wgpu_render_state().device.clone(), renderer.get_wgpu_render_state().queue.clone())
                    };
                    ...
                    if let Some(selected_model_mut) = renderer.get_models_mut().get_mut(model_index) {
                        ui.add(
                            egui::DragValue::new(&mut selected_model_mut.get_transform_mut().get_position_mut().x)
                                .speed(0.1),
                        );
                        ui.add(
                            egui::DragValue::new(&mut selected_model_mut.get_transform_mut().get_position_mut().y)
                                .speed(0.1),
                        );
                        ui.add(
                            egui::DragValue::new(&mut selected_model_mut.get_transform_mut().get_position_mut().z)
                                .speed(0.1),
                        );
                        ui.add(
                            egui::DragValue::new(&mut selected_model_mut.get_transform_mut().get_scale_mut().x)
                                .speed(0.1),
                        );
                        ui.add(
                            egui::DragValue::new(&mut selected_model_mut.get_transform_mut().get_scale_mut().y)
                                .speed(0.1),
                        );
                        ui.add(
                            egui::DragValue::new(&mut selected_model_mut.get_transform_mut().get_scale_mut().z)
                                .speed(0.1),
                        );
                        let selected_model_clone = selected_model_mut.clone_untextured(&device, &queue);
